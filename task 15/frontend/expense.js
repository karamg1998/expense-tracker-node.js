...
window.addEventListener('DOMContentLoaded',async (e)=>{
 ...
 try{
    ...
    await axios.get('http://localhost:3000/getexpense?page=1',{headers:{'Authorization':token}})
    .then(expense=>{
  ...
  for(var i=0;i<p.length;i++)
  {
    p[i].addEventListener('click', async(e)=>{
      ...
      try{
        ...
        await axios.get(`http://localhost:3000/getexpense?page=${page}`,{headers:{'Authorization':token}})
        .then(expense=>{
          let exp=expense.data.data;
